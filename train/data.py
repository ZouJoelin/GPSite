...
class ProteinGraphDataset(data.Dataset):
    def __init__(self, dataset, index, args, task_list, radius=15):
    ...
    def _featurize_graph(self, idx):
        name = self.IDs[idx]
        with torch.no_grad():
            X = torch.load(self.dataset_path + "pdb/" + name + ".tensor")
            ...
            if len(self.task_list) > 1:
                for i in range(len(self.task_list)):
                    y_task = self.dataset[name][1][i]  # multi-task: .pickle file that load dataset from must be consistent with self.task_list
                    if y_task:
                        y.append(y_task)
        ...
        graph_data = torch_geometric.data.Data(name=name, seq=seq, X=X, node_feat=pre_computed_node_feat,
                                              edge_index=edge_index, y=y, y_mask=y_mask)
        return graph_data
